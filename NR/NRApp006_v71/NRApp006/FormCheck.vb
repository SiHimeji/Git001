Public Class FormCheck
    ...
    Dim array料金チェック() As String = {
        "ステータス", "訂正依頼内容", "メモ", "出庫", "完了", "訂正更新日", "チェック", "確認完了日",
        "点検受付番号", "ｄｍ番号", "ステータス名",
        ...
        "点検状態区分名称", "修理状況", "保証規定区分", "請求先会社", "請求先名", "請求先電話", "回収金額", "価格指示理由", "更新日", "製品名"}
    ...
    Dim arrary2024() As String = {
        "ステータス", "訂正依頼内容", "メモ", "出庫", "完了", "訂正更新日", "チェック", "確認完了日", "点検受付番号", "ｄｍ番号",
    ...
    Private Sub 安心プランチェック1()
        ...
        For ro = 0 To Me.DataGridView1.Rows.Count - 2
            ...
            If Me.DataGridView1.Rows(ro).Cells(Getarray安心プラン("技術料")).Value.ToString() <> "0" Then
                Me.DataGridView1.Rows(ro).Cells(0).Style.BackColor = Color.Red
    ...
    Private Sub 料金チェック1()
        ...
        For ro = 0 To Me.DataGridView1.Rows.Count - 2
            ...
            End If
                ...
                Me.DataGridView1.Rows(ro).Cells(0).Style.BackColor = Color.Red
                Me.DataGridView1.Rows(ro).Cells(8).Style.BackColor = Color.Red
            ...
            Select Case Me.DataGridView1.Rows(ro).Cells(Getarray料金チェック("保証規定区分")).Value.ToString()                '保証規定区分
                ...
                Case "あんしん点検"
                    ...
                    'Case "法定・あんしん点検出張料ＣＰ"
                    'If Me.DataGridView1.Rows(ro).Cells(Getarray料金チェック("出張料")).Value.ToString() <> "0" Then     '出張料
                    ' Me.DataGridView1.Rows(ro).Cells(0).Style.BackColor = Color.Red
                    'Me.DataGridView1.Rows(ro).Cells(Getarray料金チェック("点検受付番号")).Style.BackColor = Color.Red
                    'Me.DataGridView1.Rows(ro).Cells(Getarray料金チェック("出張料")).Style.BackColor = Color.Red
                    'End If
...
#Region "出張料重複チェック"
    ...
    Private Sub 出張料重複チェック()
        ...
        strSQL &= " left OUTER JOIN " & schema & "t_teisei t2"
        strSQL &= " on t1.点検受付番号 = t2.点検受付番号 "
        strSQL &= " LEFT OUTER JOIN " & schema & "t_check t3"
        strSQL &= " on t1.点検受付番号 =t3.点検受付番号 "
...
#Region "非表示"
    Private Sub Button正常を削除_Click(sender As Object, e As EventArgs) Handles Button正常を削除.Click
        ...
        If Me.ComboBox項目.Text = "キャンペーン2024" Then
            co = 8
        Else
            co = 0
        End If
