...
#Region "DB関係"
    ...
    Public Function GetZeiRitu(No As String)
        ...
        Dim dt As DataTable
        Dim strSekouYMD As String = String.Empty
        Dim classOracle As New ClassOracle()

        GetZeiRitu = 10

        strSQL = "SELECT Y5_EXEC_MONTH  FROM E_Y5GUR WHERE Y5_ID ='" & No & "'"
        dt = classOracle.SetDataTable(strSQL)

        If dt.Rows.Count = 1 Then
            If dt.Rows(0).Item(0).ToString.Trim <> "" Then
                strSekouYMD = DateTime.Parse(dt.Rows(0).Item(0).ToString).ToString("yyyy/MM/dd")
            End If
        End If
        If strSekouYMD <> "" And strSekouYMD < "2019/10/01" Then
            GetZeiRitu = 8
        End If
    End Function
