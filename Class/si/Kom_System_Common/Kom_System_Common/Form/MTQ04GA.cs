...
{
    public partial class MTQ04GA : Form
    {
        private const int DEFAULT_SERACH_LIMIT = 1000;
        private const string SEARCH_LIMIT = "MTQ04GA_SearchLimit";
        ...
        /// <param name="e"></param>
        private void MTQ04GA_Load(object sender, EventArgs e)
        {
            // 初期設定
            // 表示制限件数
            ...
            dgv_Shizai.AutoGenerateColumns = false;
        ...
        /// <param name="sender"></param>
        /// <param name="e"></param>
        private void MTQ04GA_KeyDown(object sender, KeyEventArgs e)
        {

            if (e.KeyCode == Keys.Enter)
            {
                if (dgv_Shizai.Focused)
                {
                    if (dgv_Shizai.CurrentRow.Index >= 0)
                    {
                        this.SelectData();
                    }
                }
        ...
        {
            string zaicd = dgv_Shizai.Rows[e.RowIndex].Cells["ZAICD"].Value.ToString();
        ...
        private void Search()
        {
            ...
            sb.AppendLine("    ROW_NUMBER() OVER(ORDER BY ZAICD) AS RNUM");
            sb.AppendLine("    , KMZAIR.ZAICD ");
            ...
            sb.AppendLine("WHERE ");
            sb.AppendLine("    KMZAIR.DEFLG = 0 ");
            sb.AppendLine("    AND KMMEIS.DEFLG = 0 ");

            if (!string.IsNullOrEmpty(sh_Zaicd.Text.Trim()))
            {
                // 資材コード
                sb.AppendLine("AND KMZAIR.ZAICD LIKE @ZAICD");
                param.Add("@ZAICD", ($"{sh_Zaicd.Text.Trim()}%", SqlDbType.VarChar));
            ...
            {
                // 資材区分
                sb.AppendLine($"AND KMZAIR.ZAIKBN IN ({String.Join(",", inShizaiKbn)})");
            ...
            if (!string.IsNullOrEmpty(sh_Name1.Text.Trim()))
            {
                ...
                param.Add("@NAME1", ($"%{sh_Name1.Text.Trim()}%", SqlDbType.VarChar));
            ...
            if (!string.IsNullOrEmpty(sh_Name2.Text.Trim()))
            {
                ...
                param.Add("@NAME2", ($"%{sh_Name2.Text.Trim()}%", SqlDbType.VarChar));
            ...
            if (!string.IsNullOrEmpty(sh_Name3.Text.Trim()))
            {
                ...
                param.Add("@NAME3", ($"%{sh_Name3.Text.Trim()}%", SqlDbType.VarChar));
            }

            sb.AppendLine("ORDER BY ZAICD");
            ...
            try
            {
                ...
                {
                    dgv_Shizai.Focus();
                    dgv_Shizai.CurrentCell = dgv_Shizai.Rows[0].Cells[1];
                }
            }
            catch (Exception ex)
            {
        ...
        {
            if (dgv_Shizai.CurrentCell == null)
            {
            ...
            this.DialogResult = DialogResult.OK;
            this.ResultValue = dgv_Shizai.CurrentRow.Cells["ZAICD"].Value.ToString();
        }
